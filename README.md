...
| `FALLBACK_ADDRESS` (var) | Email forward target for oversized (>100 KB) and failure cases. Currently `you@example.com` |
...
5. **Email oversized** — Send a >100 KB email to `anything@oob.example`. A row with `raw_eml = "sent_to_fallback"` appears; the original lands in the fallback inbox. (Attachments alone do not trigger the fallback any more — only size does.)
