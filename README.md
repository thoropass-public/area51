...
- **`ui.jsx`** — shared utilities. Defines:
  ...
  - **Edge session guard** (`isEdgeAuthResponse` + `reloadForExpiredSession`) — the dashboard has no auth of its own; access is enforced at the network edge (Cloudflare Access), whose session cookie expires on its own schedule. A tab left open past that point keeps rendering but every `/api/*` call gets a 302 to the Access login host instead of JSON. Every API call therefore goes out with `redirect: "manual"` (a same-origin `/api/*` call never legitimately redirects), and a response that came from the edge auth layer rather than our API — an **opaque redirect**, a **401/403** (our API issues neither), or an **ok response whose body isn't the expected content type** (some Access configs serve the login page with a 200) — triggers `window.location.reload()` after ~900 ms, with a `session expired — reloading…` toast in the meantime. The reload is a *navigation*, so it follows the Access hop properly and either silently re-mints the cookie via the IdP or lands on the login screen. Guarded by a `area51:sessionReloadAt` sessionStorage timestamp: at most one auto-reload per 15 s, so a page that still can't reach the API surfaces the error instead of reload-looping. Both `apiFetch` and `getEmailRaw` (which bypasses `apiFetch` for its non-JSON body) run the check. Genuine network failures still throw normally — only edge-auth-shaped responses reload. See [§15.11](#1511-auto-reload-on-an-expired-edge-session).
...
- **`tabs.jsx`** — the three list tabs and their detail modals.
  ...
  - **Conversation grouping** (`groupEmails` + `EmailGroupRow` + `EmailGroupView`) — **always on and enforced (no toggle)**: every set of ≥2 loaded rows sharing the **exact** `(from_addr, subject)` pair collapses into one **group row** — recipient (`to_addr`) is intentionally *not* part of the identity, so one sender blasting many catch-all aliases with the same subject folds into a single group. Grouping is **global** (all matching loaded rows fold in, positioned at the newest member's `ts`) and a pure client-side view transform over the loaded rows. A group row shows the latest ts + from + subject and carries the **same read/unread treatment as any row** (unread if any loaded member is unread). Three things mark it as a group, none of which collides with the two channels already spoken for (the left 6px edge is pin ribbons; the background is read/unread): its leftmost **stack icon** (vs the envelope); a **To cell reading `<latest recipient>, and more`** — the newest member's `to_addr` (`item.to_addr` from `groupEmails`, whose input is ts-DESC) followed by a fixed suffix, where the address truncates but the suffix never does (`.group-to` is flex with no gap, not `.cell-trunc`, so the comma hugs the address and the part carrying the meaning survives a narrow column); and **`.group-stack`**, three fanned "page edge" bars on the row's **right** edge, absolutely positioned inside the row so they never bleed into neighbouring rows. An empty recipient renders as `(no recipient), and more`. A group is always ≥2 messages, so the suffix is true by construction and asserts nothing about unloaded rows. There is no chevron and no count. Two actions: clicking the **stack icon** opens a confirm to **toggle the group's read state** (a read group → unread, an unread group → read) — a bulk `PATCH /api/emails` (see [§7](#7-http-api-contract-pages-functions)) that sets `read` on the group's DB rows. **When a search/starred filter is active the update is scoped to that filter** (only the matching rows — mirroring the filtered drill-in); with no filter it covers the whole from+subject group across all recipients, including rows not currently loaded (no star toggle on the group). The confirm formats the sender and subject distinctly (`.confirm-em` / `.confirm-subj`) and says whether it's filter-scoped. Clicking **anywhere else drills in**: the tab swaps to `EmailGroupView`, a self-contained, **server-backed** list of that exact from+subject — **further narrowed by the active search terms + starred filter** (additive server-side: from+subject `AND` the same from/to/subject search `AND` starred), so a filtered drill-in shows only the members the filter catches (e.g. just the recipients hit by a pinned term); with no filter it's every message across all recipients (via `GET /api/emails?from_eq&subject_eq[&search&starred]`, its own cursor + Load More), with a **← Back** control and the from+subject in the header, plus the same **"X loaded" count and refresh button** as the main list (the refresh re-fetches the group). Inside the drill-in each message is a normal `EmailRow` (read/star/open-modal all work, and each shows its own recipient); the already-loaded root rows are irrelevant to the fetch — it re-fetches authoritatively. **Back** hands the drill-in's member rows up so **read *and* starred** changes made inside are synced onto the matching root rows by id (no refetch) — otherwise the collapsed group row would show stale state. Under the **Starred** filter it refetches instead, since star changes inside the drill-in can add/remove rows from that filtered view. Singletons at the root open the modal directly. There is intentionally **no count** on the group row (an accurate total would need a per-group `COUNT(*)`, the per-row-billing pattern [§15.1](#151-no-row-counts-anywhere-in-the-ui) avoids) — the To cell's `| and more` carries the "this is more than one message" signal instead, using only what's already loaded.
  - **Fill-to-50 pagination** (`DISPLAY_TARGET`, `displayedCount`) — because grouping can collapse a 50-row page into a handful of displayed rows, `fetchFirst`/`loadMore` **loop**: they keep paging (cursor on the underlying row `ts`, since grouping is view-only) until at least ~50 rows are *displayed* (groups counted as 1) or the data runs out. **No cap** — a spam-dominated load may pull several pages in one go, bounded only by the table. Because each fill is multi-request, a **monotonic request id** (`reqSeq`) guards against races: every fresh `fetchFirst` bumps it, and any in-flight `fetchFirst`/`loadMore` checks it after each page and discards its result if superseded — so a fetch started *before* a filter/pin change can't land late and clobber the filtered view.
...
| `PATCH` | `/api/emails` | **Bulk set read state for a group.** Body: `{from_addr, subject, read, search?, starred?}` (`subject` may be `''`; `read` boolean; `search` is an array of terms). Sets `read` on rows matching `from_addr = ? AND COALESCE(subject,'') = ?`, **plus the same additive `search`/`starred` filter** when supplied — so the update is scoped to the active filter (only matching rows), or the whole group across all recipients when no filter is sent. Returns `{ok, updated: <count>}`. Dashboard-only writer; powers the group row's stack-icon read/unread toggle. (Distinct from the per-email `PATCH /api/emails/[id]` below.) |
...
| A long-idle tab errors on refresh, then works after a manual page reload | The edge (Cloudflare Access) session cookie expired, so `/api/*` is answered with a redirect to the login host | Expected — the dashboard now detects this and auto-reloads once (see [§5.1](#51-frontend-no-build-step) / [§15.11](#1511-auto-reload-on-an-expired-edge-session)). If it *doesn't* reload, check devtools Network for the `/api/*` call: an opaque redirect / `cloudflareaccess.com` hop is a session expiry; anything else is a real API failure. Lengthen the Access app session duration to make it rarer. |
| The tab reloads repeatedly | Every API call is being answered by something that isn't our API | The 15 s `area51:sessionReloadAt` cooldown caps this at one reload per window, so a loop means the API is genuinely unreachable (Pages down, D1 binding missing → HTML error page). Check the Pages deployment and bindings. |
...
**File hosting is framed to agents as a capability they can request, not a wall.** Agents cannot upload — that stays a human action in the dashboard ([§15.12](#1512-file-backed-endpoints-server-owned-response-separate-bucket)) — but an agent that only learns "uploads are refused" will quietly route around the feature or stall. So the preamble leads with the capability (*the platform can serve an arbitrary uploaded file from a black hole URL*) and then gives the request protocol: ask the operator for a specific `/-/` path, name the file, point them at **Endpoints → New endpoint → attach under FILE**, have them confirm the path back, then hand out `https://<domain><path>`.

Three supporting facts are repeated across the preamble and the `autopilot_endpoints_*` descriptions, because each one has its own silent failure mode:

- **An empty `body` next to a `file` object doesn't mean "returns nothing"** — the bytes are in object storage. An agent that missed this would try to "fix" a working endpoint and clobber a staged payload.
- **Don't base64 a binary into `body`** — the worker serves that column verbatim, so the target would receive base64 text, not a file. This is the workaround an agent reaches for first.
- **Don't ask the operator for anything expressible as text** — HTML, JSON, XML, JS, a DTD, an SVG are all `autopilot_endpoints_upsert` with the right `Content-Type`, no human needed. Without this the guidance over-corrects into pestering the operator for text stubs.

`autopilot_endpoints_list`'s description also tells agents to check for an already-uploaded file before asking for a new one, and the 400 returned by the upsert/delete guard spells out the dashboard path rather than just refusing — that error string is the agent's only feedback at that moment.
...
- **Grouping key is the *exact* `(from, subject)` pair, not fuzzy and *not* including recipient.** One-way capture means there are no `Re:`/`Fwd:` chains to normalize, and the flood we're fighting is an automated sender blasting the catch-all — often the *same* subject to *many* different aliases. Keying on from+subject (dropping `to_addr`) collapses that whole blast into one group, where an earlier from+to+subject key would have split it per-recipient. Exact-match on the two fields stays conservative — it won't merge unrelated senders or subjects.
...
- **No count on the group row.** An accurate total per group would require a `COUNT(*)` per group — exactly the scan-per-render pattern we rejected for tab badges. A loaded-members count (`12+`) would have been free, but it's a floor dressed up as a figure, and the number people actually want (how many recipients this blast hit) is the one we can't afford. So the group row carries **no number at all**: the To cell shows the latest member's recipient followed by `| and more`. The address is data the fill loop already holds, the suffix is true for any group by definition (grouping needs ≥2 rows), and neither says anything that a `COUNT(*)` would be needed to back. The drill-in remains where you see the actual messages.
- **Group vs. singleton is marked by shape, not colour.** Background is already the read/unread channel *and it inverts per theme* (dark: unread is the lighter surface; light: unread is the brighter page), so a group tint would fight it in one theme, and the left 6px edge belongs to pin ribbons. That leaves the right edge, where `.group-stack` draws three fanned bars in `--n3` — a shape cue, so no per-theme handling, and absolutely positioned inside the row so a group sitting next to a normal email row stays tidy.

  Heavier treatments were prototyped and dropped: a "second sheet" peeking below the row (bled into the next row), small overlapping cards at the right edge, and the whole row drawn as the top card of a right-fanned stack. The last two rendered depth more literally, but both read as decoration bolted onto a dense log view. Three bars plus the icon and the To cell turned out to be enough — the bars only have to say "not a single message", not simulate a deck.
- **The drill-in is server-backed and authoritative, ignoring loaded rows.** Clicking a group fires a fresh exact-pair query (`from_eq`/`subject_eq`) paginated on its own, returning that sender+subject across *all* recipients. This is the one place we *do* filter the table by content, but it's bounded (one group, `LIMIT 50` + Load More) and, for a contiguous burst, the matches are clustered so it's cheap. It means the root fill-loop never has to fully load a giant group to be correct. (Consequence: drilling in from a recipient-filtered search still shows the group's other recipients — recipient isn't part of group identity.)
...
### 15.11 Auto-reload on an expired edge session

The dashboard holds no session of its own — the only session in the system is the edge one (Cloudflare Access). When it expires, an open tab looks healthy until the next `/api/*` call, which the edge answers with a 302 to its login host. Under fetch's default redirect mode the browser follows that cross-origin hop, gets no CORS headers, and the call rejects as a bare `Failed to fetch` — indistinguishable from a real outage, and clearing only on a manual page reload.

We fixed this in the API client rather than at the edge:

- **`redirect: "manual"` on every API call.** A same-origin `/api/*` request never legitimately redirects, so turning the 302 into an opaque-redirect response (`type === "opaqueredirect"`, `status === 0`) makes session expiry an unambiguous, *catchable* signal instead of an opaque CORS rejection.
- **Reload rather than prompt.** The reload is a navigation, which is exactly what the Access hop needs: the IdP re-mints the cookie silently (or shows the login screen if that session is gone too). A "click here to re-auth" banner would just be a manual version of the same navigation.
- **Narrow trigger.** Only edge-auth-shaped responses reload — opaque redirect, 401/403 (our API has no auth and issues neither), or an ok response whose body isn't the expected content type. A `TypeError` from a genuinely offline browser does **not** reload: reloading an offline page replaces the app with the browser's error page and loses more than it fixes.
