...
| `GET` | `/api/emails` | List. Params: `cursor` (last `ts`), `search` (**may be repeated**; each term ORs across `from_addr`/`to_addr`/`subject` via LIKE), `starred` (`1` = starred-only, from the toolbar Starred toggle). The per-term OR groups form one parenthesized **OR** group; `starred=1` is a **separate `AND starred = 1`** constraint (starred *and* matching the search, not OR); both are ANDed with the cursor. Returns `{id, ts, from_addr, to_addr, subject, read, starred}` per row. Sorted DESC by `ts`. **Drill-in mode:** if `from_eq` **and** `subject_eq` are both present, the endpoint instead returns only the exact `(from, subject)` group across all recipients — `from_addr = ? AND COALESCE(subject,'') = ?` (paginated by `cursor`); `search`/`starred` are ignored. Used by the conversation drill-in. |
| `PATCH` | `/api/emails` | **Bulk set read state for a group.** Body: `{from_addr, subject, read}` (`subject` may be `''`; `read` boolean). Sets `read` on every row matching `from_addr = ? AND COALESCE(subject,'') = ?` — the whole conversation group across all recipients, including rows not loaded in the UI. Returns `{ok, updated: <count>}`. Dashboard-only writer; powers the group row's stack-icon read/unread toggle. (Distinct from the per-email `PATCH /api/emails/[id]` below.) |
