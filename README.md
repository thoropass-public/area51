...
| `PATCH` | `/api/emails` | **Bulk mark-as-read for a group.** Body: `{from_addr, subject, read}` (`subject` may be `''`). Sets `read` on every row matching `from_addr = ? AND COALESCE(subject,'') = ?` — the whole conversation group across all recipients, including rows not loaded in the UI. Returns `{ok, updated: <count>}`. Dashboard-only writer; powers the group row's stack-icon action. (Distinct from the per-email `PATCH /api/emails/[id]` below.) |
