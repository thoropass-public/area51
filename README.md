...
- **Group vs. singleton is marked by the icon and the To cell — no extra row decoration.** Colour isn't available: background is already the read/unread channel *and it inverts per theme* (dark: unread is the lighter surface; light: unread is the brighter page), so a group tint would fight it in one theme, and the left 6px edge belongs to pin ribbons. That points at a shape cue, and several were prototyped — a "second sheet" peeking below the row (bled into the next row), bars at the right edge (nothing occluded anything, so they read as tick marks), small overlapping cards at the right edge (real occlusion, but read as a badge parked at one end), and finally the whole row drawn as the top card of a right-fanned stack (worked, but too heavy for a dense log view). All were dropped: at 44px in a dense list, the stack icon plus a To cell that visibly describes more than one message carries the distinction without adding chrome to every group row. Revisit only if the icon alone proves too subtle in practice.
