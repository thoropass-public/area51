...
| `FALLBACK_ADDRESS` (var) | Email forward target for oversized / attachment / failure cases. Currently `you@example.com` |
| `[triggers].email` | Set to `*@oob.example` so Email Routing catch-all delivers all addresses to the worker |

The HTTP route (`oob.example/*`) is configured via the Cloudflare dashboard, not `wrangler.toml`.
...
2. Application domain: `area51.ops.example` (include all subpaths).
...
Prereqs: Cloudflare account, `wrangler` CLI authenticated (`wrangler login`), Node.js installed, `oob.example` and `ops.example` zones already on Cloudflare.
...
- **Workers → area51-worker → Triggers → Routes** → add `oob.example/*` (or `*oob.example/*` for all subdomains if desired).
- **Email → Email Routing** on the `oob.example` zone → enable → set the catch-all destination to **the Worker** (`area51-worker`).
...
Bind a custom domain: **Settings → Custom domains** → `area51.ops.example`.
...
- `oob.example` zone: orange-cloud on (proxied through Cloudflare); Email Routing enabled.
- `area51.ops.example`: CNAME (or alias) pointing to the Pages project; orange-cloud on.
...
| `oob.example` returns 404 for everything | Worker not routed, OR endpoint table empty | Confirm `oob.example/*` route is bound to the worker. Confirm `SELECT * FROM endpoints` returns rows. |
