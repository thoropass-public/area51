    ...
    case 'initialize':
      return jsonRpcResult(msg.id, {
        ...
        instructions: [
          ...
          "",
          "Some endpoints serve an uploaded FILE rather than a text body — a",
          "hosted DTD, script, archive, image, or other binary payload. Those",
          "rows come back with a `file` object ({filename, content_type}) and an",
          "empty `body`: the bytes live in object storage, not in the endpoint",
          "record, so an empty body on such a row does NOT mean the endpoint",
          "returns nothing. Serve them exactly like any other endpoint — build",
          "https://<domain>/-/<path> and hand that URL to the target; it responds",
          "200 with the file's own Content-Type, inline (no attachment header).",
          "You cannot create, replace, or delete a file-backed endpoint: uploads",
          "happen only in the AREA 51 dashboard, and autopilot_endpoints_upsert /",
          "autopilot_endpoints_delete return an error on those URIs rather than",
          "destroying a payload a human staged. If you need a file hosted, ask",
          "the operator to upload it in the dashboard and tell you the path.",
          "For anything you can express as text (HTML, JSON, XML, JS, a DTD),",
          "autopilot_endpoints_upsert is still the right tool — set the",
          "Content-Type header yourself and put the content in `body`.",
        ].join(' '),
      });
