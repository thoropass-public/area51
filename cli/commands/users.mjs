// `./a51 users` — the operators of this deployment.
//
// Shapes:
...
//   ./a51 users rotate-secret <email>   issue a new key, invalidate the old one
...
export async function run(args) {
  ...
  if (action === 'rotate-secret') return rotate(ctx, arg);
...
async function rotate({ env, cf, accountId }, emailArg) {
  if (!emailArg) die('usage: ./a51 users rotate-secret <email>');
  const { ok: valid, email, reason } = normalizeEmail(emailArg);
  if (!valid) die(reason);
 ...
 *
 * The repair path for a push that failed half-way — a missing permission, an
 * inactive Zero Trust org, a network error — and the only command that touches
 * the Access application without a user having changed. It cannot do anything
...
function printKey(env, email, key, label) {
  ...
  plain(`  shown again — a lost key is replaced with \`./a51 users rotate-secret ${email}\`.`);
  plain('');
