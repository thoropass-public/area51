...
export function saveEnv(updates) {
  ...
  // writeFileSync's `mode` is only honored when it CREATES the file, and .env
  ...
  // applies either way. This file holds the Cloudflare API token, which alone
  // can rebuild or tear down the whole deployment, so it must not be readable
  // by other users on the machine. (No operator key is ever written here; those
  // live only in the D1 `users` table, as hashes.)
  try {
    chmodSync(envPath, 0o600);
  } catch {
    // A filesystem that cannot express it (a mounted share) is not worth failing
    // a deploy over, and the write itself already succeeded.
  }
  return written;
}
