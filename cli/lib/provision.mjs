// The provisioning primitives, shared by `setup`, `deploy`, `black-holes`,
// `doctor` and `destroy` directly, and by `users` through cli/lib/users.mjs.
// Every function here is idempotent: it inspects the current state
// first and only writes what is actually missing, so running setup twice is
// boring rather than destructive. Return shapes differ per helper and are not a
// contract: `ensureDatabase` and `ensureAccessList` report `created: false`,
// and others return a resource, a boolean, or nothing at all.
//
...
// returns instead of throwing. The run continues, and setup prints the list of
// manual follow-ups at the end (and exits non-zero).

import { readFileSync } from 'node:fs';
import { join } from 'node:path';
import { repoRoot } from './env.mjs';
import { ok, skip, warn, info, color } from './log.mjs';
import { isAlreadyExists, isAuthError, isHostnameOccupied } from './cloudflare.mjs';
 ...
 * likely first:
 *
 *   1. token propagation, for a token created or edited seconds ago (setup
 *      retries a few times, but a long propagation still needs a re-run),
 ...
 *      Routing Rules),
 *   3. Zone Resources scope, because a zone permission that doesn't *include*
 *      this zone throws the same error even when the checkbox is ticked,
...
export async function attempt(followUps, label, fn, fixHint) {
  try {
    ...
    const hint = fixHint || 'Fix the cause above, then re-run — steps that already succeeded report `·` and do their work again only if something changed.';
    degraded(
      followUps,
      `${label}: ${message}`,
...
export async function ensureBlackHole(cf, accountId, opts) {
  ...
  if (roles.includes('http')) {
    await attachBlackHoleDomain(cf, accountId, { hostname, workerName, zone, followUps });
  }
...
/**
 * Bind `hostname` to the Black Holes worker as a Custom Domain, freeing the name
 * first if something else holds it.
 *
 * Cloudflare refuses the bind with `[100117] ... already has externally managed
 * DNS records` whenever an address record it did not create for this Worker sits
 * at the name. That is the normal state of a domain worth taking over: a parked
 * A record, or a CNAME left behind by a Pages project or another Worker.
 *
 * Both callers of `ensureBlackHole` list those exact records back to the
 * operator and gate on a typed TAKEOVER first, and the setup banner says
 * plainly that the apex address record is replaced. Stopping here would refuse
 * what the operator just agreed to and hand back a DNS chore to do by hand. So
 * the refusal is a step rather than an end: release the name, then bind again.
 *
 * Nothing is released speculatively. The clear-out runs only after Cloudflare
 * has said the name is occupied, so a re-run, where the record is already ours
 * and the upsert simply succeeds, never deletes anything.
 */
async function attachBlackHoleDomain(cf, accountId, { hostname, workerName, zone, followUps }) {
  const manual = `Bind it by hand: dashboard → Workers & Pages → ${workerName} → Settings → Domains & Routes\n→ Add → Custom Domain → ${hostname}.`;
  const attach = () => cf.attachWorkerDomain(accountId, { hostname, service: workerName, zoneId: zone.id });
  const degrade = (err, leftovers = []) => degraded(
    followUps,
  ...
  try {
    ...
    warn(`${hostname} is held by something else. Releasing it, which is the takeover you confirmed.`);
  }

  const leftovers = await releaseHostname(cf, accountId, zone, hostname, workerName);

  // A release can take a moment to register, a detached Pages custom domain
  // especially, and until it does Cloudflare answers the same [100117]. Two
  // short retries keep this to one command instead of asking for a re-run.
  for (let attempt = 0; ; attempt++) {
    try {
      await attach();
      ok(`${color.bold(hostname)} → Custom Domain on ${workerName} ${color.dim('(replaced what was there)')}`);
      return;
    } catch (err) {
      if (isHostnameOccupied(err) && attempt < 2) {
        await sleep(2000);
        continue;
      }
      degrade(err, leftovers);
      return;
    }
  }
}
...
/**
 * Free `hostname` so a Custom Domain can be created on it. Returns one note per
 * thing that could NOT be released, which the caller folds into its follow-up if
 * the bind still fails. "[100117] externally managed DNS records" on its own
 * does not tell an operator which product to go and click.
 *
 * The order is deliberate: ask each product to give the name up BEFORE touching
 * DNS, then sweep whatever address records are left. Cloudflare manages the DNS
 * record it creates for a Pages custom domain or a Worker Custom Domain, so
 * deleting that record behind its owner's back leaves the owner still claiming a
 * hostname it no longer serves, and the DNS API does not reliably allow it
 * anyway. Step 3 is for records nobody manages: a parked A record, a CNAME to a
 * foreign host, or a leftover from an account this token cannot see.
 */
async function releaseHostname(cf, accountId, zone, hostname, keepService) {
  const leftovers = [];

  // 1. A Custom Domain on another Worker. Listed unfiltered on purpose: the
  //    whole point is to find the service that is NOT ours.
  try {
    const bound = (await cf.listWorkerDomains(accountId)) || [];
    for (const d of bound.filter((d) => d.hostname === hostname && d.service !== keepService)) {
      try {
        await cf.detachWorkerDomain(accountId, d.id);
        ok(`released ${hostname} from the worker ${color.bold(d.service)}`);
      } catch (err) {
        leftovers.push(
          `${hostname} is still a Custom Domain on the worker ${d.service} (${err.message}).\n` +
          `Detach it: dashboard → Workers & Pages → ${d.service} → Settings → Domains & Routes.`,
        );
      }
    }
  } catch (err) {
    leftovers.push(`could not list this account's Worker Custom Domains (${err.message}).`);
  }

  // 2. A Pages project's custom domain, the usual holder on a zone that used to
  //    serve a site. Detaching it removes Cloudflare's managed CNAME too.
  try {
    const projects = (await cf.listPagesProjects(accountId)) || [];
    for (const p of projects.filter((p) => (p.domains || []).includes(hostname))) {
      try {
        await cf.deletePagesDomain(accountId, p.name, hostname);
        ok(`released ${hostname} from the Pages project ${color.bold(p.name)}`);
      } catch (err) {
        leftovers.push(
          `${hostname} is still a custom domain on the Pages project ${p.name} (${err.message}).\n` +
          `Detach it: dashboard → Workers & Pages → ${p.name} → Custom domains.`,
        );
      }
    }
  } catch (err) {
    leftovers.push(`could not list this account's Pages projects (${err.message}).`);
  }

  // 3. Whatever address record is left. Re-read rather than reuse a listing from
  //    before the two detaches above: they delete records of their own, and
  //    deleting one twice reads as a failure that isn't one.
  let records = [];
  try {
    records = (await cf.listDnsRecordsByName(zone.id, hostname))
      .filter((r) => ADDRESS_RECORD_TYPES.includes(r.type));
  } catch (err) {
    leftovers.push(`could not re-read the DNS records at ${hostname} (${err.message}).`);
  }
  for (const r of records) {
    try {
      await cf.deleteDnsRecord(zone.id, r.id);
      ok(`deleted the ${r.type} record ${r.name} → ${r.content}`);
    } catch (err) {
      leftovers.push(
        `the ${r.type} record ${r.name} → ${r.content} is still there (${err.message}).\n` +
        `Delete it: dashboard → ${zone.name} → DNS → Records. The token needs Zone · DNS:Edit.`,
      );
    }
  }
...
export async function ensureEmailRouting(cf, zone, workerName, followUps, hostname = zone.name) {
  ...
  // and that legitimately means "disabled". An AUTH error, though, is the real
...
async function waitForAccessOrg(cf, accountId, { attempts = 4, delayMs = 1500 } = {}) {
  for (let i = 0; i < attempts; i++) {
    ...
    try {
      ...
      if (org) return org;
    } catch { /* keep polling; this is transient during propagation */ }
  }
  return null;
}
 ...
 * rather than replaced. It is the migration path for a deployment whose Access
 ...
 * operator list, authenticating with One-time PIN (an emailed code, with no
...
export async function ensureAccess(cf, accountId, opts) {
  ...
  if (!org) {
    ...
    try {
      ...
      // and that first-run race is what made the app creation below fail until a
  ...
  // reliance on emergent behavior, and the dashboard is shut whatever Cloudflare
