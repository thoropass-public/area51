...
//
// Its output is also captured rather than inherited, as runWrangler shows. Wrangler
 ...
 *
 * `input` is piped to stdin rather than argv. No caller passes it today. It is
 * left in place so that anything credential-shaped, if it ever comes back, has
 * a path in that never touches a command line.
 */
export function runWrangler(args, { cwd = repoRoot, env = {}, input, label } = {}) {
  const bin = requireWrangler();
  trace(`$ wrangler ${args.join(' ')}`);

  // --verbose inherits instead of capturing, so the raw output arrives live
  // rather than after the fact.
  const inherit = isVerbose();
