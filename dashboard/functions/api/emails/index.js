...
// Append the fuzzy search + starred filter to a WHERE builder. Each search term
...
async function listEmails({ request, env }) {
  ...
  // Fuzzy search + starred filter — applied in both the normal list and the
...
// Bulk set read state for a conversation group — every row sharing the exact
// (from_addr, subject) pair. When the caller passes the active search/starred
