// Shared machinery for the dashboard: the API client, the edge-session guard,
// and every cross-tab primitive (toasts, modal, confirm, blacklist context,
// debounce, pinned filters, the domain cache, clipboard, icons, formatters).
// `tabs.jsx` and `app.jsx` are built on top of it, so this file loads first.
//
// There is no bundler and no module system here. Babel-standalone transpiles
// these files in the browser and cannot resolve imports, so everything other
// files need is published on `window` in one Object.assign at the bottom. That
// list is the de-facto export surface: if you add something used elsewhere, add
// it there too or the other file gets `undefined` at runtime with no build-time
// warning. See docs/internals/dashboard.md.
...
// can't reach the API, the reload isn't the fix, so surface the error instead
// of looping.
...
//   - an explicit 401/403 (our API never issues either, since it has no auth), or
...
// non-empty term becomes its own ?search=... and the server ORs them, so a row
// matching ANY term comes back. That is what makes pins broaden the result set
// rather than narrow it, and why the toolbar badge reads "N pinned · OR".
// (The starred filter is the exception: it ANDs.)
const qs = (params) => {
...
const API = {
  ...
  // still sent, since qs() drops empty-string values and that would break the query.
...
// source of truth, and the frontend just mirrors the lists for snappy
...
// Yellow is absent on purpose: ":star:" was once a pin and owned that color.
// It is no longer one: the Emails toolbar has a dedicated Starred toggle, and
// tabs.jsx drops any ":star:" left in a returning browser's storage. The three
// ":star:" branches below are defensive handling for exactly that stale value,
// not a live feature. Reclaim yellow here if the reserved slot stops earning it.
...
function usePinnedFilters(tab) {
  ...
  const addPin = useCallback((value) => {
    ...
    // Assign a color the first time a value is pinned. A legacy ":star:" still
    // maps to yellow so a stale one renders sanely until it is purged;
    // everything else draws from the palette, preferring an unused color.
    setColors((prev) => {
...
// Which text pins match a row's field, in pin order. A legacy ":star:" pin is
// skipped so it never draws a ribbon segment: starring is not a text match, and
// it has its own per-row button and toolbar toggle. Live pins can no longer be
// ":star:"; this only guards one left in storage.
