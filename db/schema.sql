...
-- ─── emails ─────────────────────────────────────────────────────────────────
-- A lean index row per captured email. The full message, meaning every header,
-- both bodies and the attachment bytes, lives only in the raw .eml in R2 at
...
-- it is minted and never recoverable, so `./a51 users rotate-key` issues a new
