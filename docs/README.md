...
- **[guides/](#guides)** for task-oriented reading. "How do I do this?"
- **[reference/](#reference)** for lookup. "What is the exact value, flag or route?"
- **[internals/](#internals)** for mechanism. "How does this actually work?"
...
| [configuration](reference/configuration.md) | Every `.env` value: what reads it, what changing it costs, whether it needs a redeploy. Plus the constants that are deliberately hardcoded |
...
- **Catcher.** The Black Holes worker, the one Worker every black hole is bound
  to. "Catcher" and "Black Holes worker" mean the same thing; the short form is
  used throughout because it reads better in prose about traffic.
- **Capture.** A stored request or email.
- **Autopilot.** The agent-facing worker, and the `/-/*` namespace it is confined
  to.
- `./a51 …` is the CLI at the repository root. Anything routine has a command;
