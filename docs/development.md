...
npx wrangler tail area51-black-holes   # watch what it does
...
interactively and age-based for both tables. Watch a real scheduled run in the
worker's Logs in the Cloudflare dashboard.
...
| `cli/lib/provision.mjs` | The idempotent provisioning primitives. Imported directly by `setup`, `deploy`, `black-holes`, `doctor` and `destroy`, and by `users` through `cli/lib/users.mjs` |
...
| `cli/lib/users.mjs` | Operators: key minting and hashing, the D1 `users` table, and the Zero Trust allow-list projection. The most credential-sensitive file in the CLI |
...
- Provisioning uses the REST API, not wrangler, so it can inspect state and be
  idempotent. Wrangler is only for uploading code.
- Every provisioning helper is **GET-then-act**: it inspects current state before
  writing. Return shapes are per-helper, not a shared contract; only
  `ensureDatabase` and `ensureAccessList` report `created: false`.
...
- Never print an operator key except where they explicitly need to copy it (the
  MCP registration command, at the moment the key is minted).
