...
the cause and re-running converges. The parts that already worked re-report as
`·` and do no work.
...
|---|---|---|
| Black hole | `<black-hole.tld>`, the zone apex, always | **Public.** Targets reach it. Serves your endpoints, captures every request and every address. |
| AREA 51 | `area51.<black-hole.tld>` | Behind Cloudflare Access. |
| Autopilot | `autopilot.<black-hole.tld>` | Public but useless without an operator key; every route answers 401. |

The black hole always carries **both roles** (`http` and `mail`), which gives you
two callback surfaces on one domain:

```
HTTP callbacks    https://<black-hole.tld>/<anything>
Email callbacks   <anything>@<black-hole.tld>
```
...
callback address the tool prints works. The alternative is a subdomain black hole
with no MX behind it, which bounces the target's mail back at the target and
shows you nothing, which reads as "the vulnerability didn't fire."
...
```bash
./a51 black-holes add <http-only.black-hole.tld> http  # HTTP catcher, no mail records touched
./a51 black-holes add <sub.black-hole.tld> mail        # email capture on a subdomain
./a51 black-holes add <other-black-hole.tld>           # asks which roles you want
```
...
existing catch-all already covers every enabled subdomain, so it delivers to
the catcher. `<anything>@<sub.black-hole.tld>` is captured like any other address.
...
Once you consent, the apex is really cleared. If a Pages project or another
Worker on this account holds the record, setup detaches the hostname from that
product. If it is a plain DNS record, setup deletes it. Then it binds the
catcher. Anything setup could not release is listed in the follow-ups, with the
dashboard click-path for it.

If `area51.<zone>` or `autopilot.<zone>` already holds a DNS record that is not
part of this deployment, setup names the record, skips that hostname's own steps
and provisions everything else, then lists the conflict as a follow-up (exit
`2`). Delete the record or set the hostname in `.env`, then re-run. Records this
...
Asks for **your** email address, and only yours, because setup provisions one
operator.
...
The same address gets an Autopilot API key, minted in step 5 once the database
...
That is what makes `./a51 users add` take effect with no redeploy at all.
...
| Someone joined or left | `./a51 users add <email>` / `./a51 users remove <email>`, which covers both doors at once |
