...
| Nothing arrives, and routing looks right | The zone apex has to be the mail domain — Email Routing catch-alls are zone-wide | Send to `*@<zone>`, not `*@<subdomain>` |
...
| Emails I need were deleted | Retention is age-based and unattended | Star messages you want kept — starred email is exempt forever |
...
| Uploaded payloads accumulate | They are never aged out — `endpoints` has no timestamp | Delete the endpoints, or add a lifecycle rule to the files bucket |
