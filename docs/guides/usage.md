...
- Everything the dashboard shows, an [agent can read](#hand-the-engagement-to-an-agent)
  through Autopilot.
...
Requests older than N days, emails older than N days (rows and their stored
messages together), or every agent-staged `/-/*` endpoint. Then delete the
