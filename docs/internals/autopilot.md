...
- Only the hash is stored. A key is shown once, when minted, and is not
  recoverable; `./a51 users rotate-key <email>` issues a replacement.
...
SSE, since every tool completes fast). `serverInfo.version` is `1.3.0`. It
versions the **MCP surface**, not the product, and is bumped whenever a tool or
its description changes ([development](../development.md#common-tasks)), so it
deliberately does not track `package.json`.

| Tool | Wraps | Arguments |
|---|---|---|
| `requests_recent_1hr` | `GET /requests` | none |
| `emails_recent_1hr` | `GET /emails` | none |
...
| `autopilot_endpoints_list` | `GET /autopilot/endpoints` | none |
...
**Endpoints → + New endpoint → drop the file on "Serve a file" → Create**, have
them confirm the path,
...
system prompt. The key lives in the client's own config, not in the
conversation.

`./a51 users add <email>` prints this exact line, filled in, at the moment the
key is minted, which is the only moment it is readable.
...
# minted. If it is lost: ./a51 users rotate-key <your email>
KEY=<key_id>_<secret>
...
# to check anyone's key. That is an infrastructure fault, not a bad credential.
curl -sS -o /dev/null -w '%{http_code}\n' \
...
`./a51 doctor` runs the two unauthenticated ones automatically: no key, then a
bogus key, expecting `401` from both. The others need a real key, which `doctor`
deliberately does not have (none is stored anywhere), so run those by hand with
your own.

## Log events

One JSON object per line, `console.log` for success and `console.error` for
failure, same convention as the other two workers. Read them in the Cloudflare
dashboard (Workers & Pages → *worker* → Logs), or stream them live with
`npx wrangler tail area51-autopilot`.

| Event | When |
|---|---|
| `agent_hit` | top of every request, before auth |
| `agent_authorized` | auth succeeded; records `key_id` and `email`, never the key. This is what makes a call attributable |
| `agent_unauthorized` | refused, with `reason` (`no_bearer`, `malformed_key`, `unknown_key_id`, `bad_secret`) and `status` |
| `agent_auth_unavailable` | D1 unreachable, so no key could be checked → `503`, not `401` |
| `agent_d1_requests` / `agent_d1_emails` | a read completed, with row count and duration |
| `agent_email_raw_ok` | a raw `.eml` was served |
| `agent_email_raw_denied` | the id was unknown or outside the 60-minute window |
| `agent_email_raw_missing` | the row was in the window but its R2 object was gone |
| `autopilot_upsert` / `autopilot_delete` | a `/-/*` endpoint was written or removed |
| `autopilot_file_backed_refused` | upsert or delete refused because the URI serves an uploaded file |
| `mcp_request` | an MCP JSON-RPC call arrived, with its `method` |
| `mcp_tool_failed` | a `tools/call` handler threw |
| `agent_unhandled_error` | anything else that escaped to the top-level catch → `500` |

Logs are persisted and queryable in the Cloudflare dashboard
