...
catch-all rule delivers to it; `./a51 setup` and `./a51 black-holes add … mail` set
...
- **Memory ceiling.** The whole message is buffered once, to write it to R2 and parse
...
`console.error` for failure, via the `log()` / `logErr()` helpers. Read them in
the Cloudflare dashboard (Workers & Pages → *worker* → Logs), or stream them
live with `npx wrangler tail area51-black-holes`.
...
| `blacklist_load_failed` | a blacklist query threw. The worker **fails open** and uses an empty set, so capture continues but nothing is being filtered until the next cache miss succeeds. Worth alerting on |

```bash
npx wrangler tail area51-black-holes --format=json | jq 'select(.event == "email_stored")'
```
...
- Blacklist your own IP, hit a black hole within the hour, and expect `403` with
  no new row plus a `http_rejected_blacklist` line in the logs.
