...
### `ui.jsx` — shared machinery

- **`API`** — the JSON client, one method per route, always same-origin `/api/…`.
- **Expired-session handling** — see [below](#expired-session-handling).
- **`ToastProvider` / `useToast`** — transient bottom-right notifications.
- **`ConfirmProvider` / `useConfirm`** — promise-returning confirm dialog, awaited
  by every destructive action.
- **`Modal` / `ModalHead`** — modal shell with Escape and backdrop-click closing.
- **`useDebouncedValue`** — 300 ms debounce for search inputs.
- **`usePinnedFilters(tab)`** — persistent search pins (below).
...
- **`App`** — tab state, keyboard shortcuts (⌘/Ctrl + 1–4), theme, and the
  `ConfirmProvider` + `ToastProvider` wrappers.
- **`TopBar`** — brand mark, tabs, and a refresh button (list tabs only) that
  bumps a `refreshTick` the active tab consumes as a `refreshKey` dependency.
- **`Home`** — the landing tab. Its orbit chips double as the **default-host
...
thrashing. A genuine network `TypeError` does **not** reload — reloading an offline
