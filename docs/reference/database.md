...
Index: `idx_requests_ts ON requests(ts DESC)` — backs newest-first cursor
pagination.
...
| `ts` | `TEXT NOT NULL` | ISO 8601 UTC. Indexed `DESC`. |
| `from_addr` | `TEXT NOT NULL` | The **`From:` header** address (`parsed.from.address`) — what the dashboard shows and what the blacklist matches. `''` if parsing failed or the message had no `From:`. The SMTP envelope sender is deliberately *not* used as a fallback; it is recoverable from the raw `.eml`. |
...
On a query error the worker uses an empty set — a database blip must not block
captures.
...
Read live by the dashboard (`/api/config/domains` — the Home host picker) and by
...
object, and its upsert/delete tools refuse file-backed URIs outright — a human
