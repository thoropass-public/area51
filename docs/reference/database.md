...
| `starred` | `INTEGER NOT NULL DEFAULT 0` | UI state, dashboard-only. Backs the Starred filter **and exempts the row from deletion**: both the cleanup worker and `./a51 purge` select `AND starred = 0`. |
...
| Column | Type | Notes |
|---|---|---|
| `domain` | `TEXT PRIMARY KEY` | The hostname, e.g. `<black-hole.tld>` |
...
`https://<domain>/-/<path>`. Managed by `./a51 black-holes`; changes apply on the
next page load or agent call, with no redeploy.

A row here does **not** by itself make a hostname work. The Custom Domain and
mail routing are the other two thirds. `./a51 black-holes add` does all three;
...
`./a51 users add` / `./a51 users rotate-key` mints it, and cannot be recovered
...
- **New table or index:** add it to `db/schema.sql` with `IF NOT EXISTS` and run
  `./a51 deploy schema`.
- **New trigger, or anything with a `BEGIN … END` body:** `./a51 deploy schema`
  **cannot apply it.** The schema is split into statements by a small parser
  (`cli/lib/sql.mjs`) that understands `--` comments, quoted strings and `;`
  terminators, but not block bodies. It would cut a trigger at the first
  internal `;` and send fragments. Apply those with
  `npx wrangler d1 execute <db> --remote --file=<path>` instead, and keep them in
  a file the CLI does not feed to the splitter. This matters because a counters
  table maintained by triggers is the sanctioned way to bring row counts back
