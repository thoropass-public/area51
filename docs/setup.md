...
| Zone | **Email Routing Rules** · Edit | enabling Email Routing and setting the catch-all |

Under *Zone Resources*, include the zone you are deploying to (or *All zones*).
...
it fails, prints the exact dashboard click-path, and setup carries on and lists
the follow-up at the end (exit code `2`).
...
- With the `mail` role: `POST /zones/{z}/email/routing/enable` (adds and locks
  the MX and SPF records), then
  `PUT /zones/{z}/email/routing/rules/catch_all` with a `worker` action pointing
  at the catcher.
...
- Creates the Pages project **with its bindings already attached** —
  `d1_databases.DB`, `r2_buckets.EML`, `r2_buckets.FILES`, for both the
  production and preview configurations. Doing this before the first upload is
  what avoids the classic "every `/api/*` call returns 500 until you add the
  binding and redeploy" trap. On an existing project the bindings are PATCHed
  instead.
- `wrangler pages deploy .` from `dashboard/`.
- `POST /pages/projects/{p}/domains` for the dashboard hostname, plus a proxied
  `CNAME` to `<project>.pages.dev`. An existing DNS record for that name is
  never overwritten — setup warns instead.
...
- `GET /accounts/{a}/access/organizations`, and if the account has no Zero Trust
  ...
  Team names are **globally unique**; if yours is taken, set `ACCESS_TEAM_NAME`
  in `.env` and re-run `./a51 access`.
- Ensures the **One-time PIN** login method exists (Access emails a code — no
  identity provider to configure).
- Creates or updates a `self_hosted` application on the dashboard hostname with
  one allow policy built from `ALLOWED_EMAILS`, `session_duration` from
  `ACCESS_SESSION_DURATION`, and `auto_redirect_to_identity` so users skip the
  login-method chooser.

*Manual equivalent:* Zero Trust → Access → Applications → *Add an application* →
Self-hosted → the dashboard hostname → policy *Allow* with an Emails or Email
domain rule.
...
| Changed who may log in | `./a51 access you@example.com,team.example.com` |
