...
// — not multipart — so the bytes stream straight into R2 and never materialise
...
async function uploadEndpointFile({ request, env }) {
  if (!env.FILES) {
    // The Pages FILES binding is added by hand in the dashboard, per environment
    // — a missing one is a deploy step that didn't happen, not a bad request.
  ...
  // The object this URI currently points at (if any) — deleted after the row is
  // successfully repointed at the new key.
