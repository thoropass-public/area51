...
// (see docs/internals/autopilot.md). Separate from the Black Holes worker so it
// can be locked behind per-operator key auth without affecting the black holes'
// public reachability.
//
...
async function authenticate(request, env) {
  ...
  // builds it. Nothing links the two at build time, since the CLI cannot be
  // imported here, so changing one means changing the other in the same commit.
  const key = bearer[1];
...
// not enough. (Every D1 row has a matching R2 object, because capture is
// all-or-nothing.)
...
const MCP_TOOLS = [
  ...
  },
  {
    ...
    description: [
      ...
      "(key→value strings). `body` is a string, served verbatim.",
      "",
      ...
      "file-backed endpoint — AREA 51 → Endpoints → + New endpoint → drop the",
      "file onto the \"Serve a file\" target, then Create — and to confirm",
  ...
  switch (msg.method) {
    case 'initialize':
      return jsonRpcResult(msg.id, {
        ...
        instructions: [
          "Autopilot is the MCP interface to this operator's AREA 51 callback",
          ...
          "AREA 51 → Endpoints → + New endpoint, enter that path, drop the file",
          "onto the \"Serve a file\" target, and press Create. Have them confirm",
          "the path back to you, then",
